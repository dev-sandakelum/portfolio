...
export function DownloadButton({ href, filename, large = false }: Props) {
  ...
  return (
    <a
      ...
      className={
        large
          ? "inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-700 active:scale-95"
    ...
    >
      {/* Download icon */}
      <svg
        className={large ? "h-4 w-4 shrink-0" : "h-3.5 w-3.5 shrink-0"}
        fill="none"
        stroke="currentColor"
        strokeWidth={2.5}
        viewBox="0 0 24 24"
      ...
      {large ? (
        <span className="max-w-[160px] truncate capitalize">{displayName}</span>
      ) : (
        "Download"
      )}
    </a>
